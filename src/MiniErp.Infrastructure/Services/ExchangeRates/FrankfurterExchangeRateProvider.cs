    ...
    : IExchangeRateProvider
{
    ...
    private static Error MapStatusCode(
    ...
    private static Error UnsupportedCurrency(CurrencyCode currency) =>
        Error.Validation(
            "ExchangeRates.ProviderUnsupportedCurrency",
            $"The CBE provider does not support currency {currency}.");

    private static Error DirectionMismatch(
        CurrencyCode currency,
        CurrencyCode baseCurrency) =>
        Error.BadGateway(
