        ...
        OperationFilterContext context)
    {
        ...
        var documentation = context.MethodInfo.Name switch
        {
            ...
            nameof(StatementsController.GetTrialBalance) => (
                ...
                """
                يجمع الأرصدة التشغيلية الحالية على دليل الحسابات من خلال `AccountMappings`، ويضيف القيود اليدوية والافتتاحية المرحلة تلقائيًا، بدون إنشاء قيود آلية محفوظة للفواتير أو السندات.

                **مصادر التجميع:** الفواتير، سندات القبض والصرف، الأرصدة الافتتاحية للأطراف والخزائن والموظفين، وتكلفة حركات المخزون. يتم استخدام الحساب المباشر في السند أولًا، ثم ربط نوع الحركة أو نوع الفاتورة، ثم يظهر المصدر غير المربوط تحت `غير مصنف`.
                ...
                استخدم `adjustmentView=BeforeAdjustments` لاستبعاد قيود التسوية فقط مع إبقاء القيود اليومية والافتتاحية، أو `adjustmentView=AfterAdjustments` لإضافة قيود التسوية المرحلة. القيود المعكوسة وقيد العكس يظهران معًا حتى يظل مسار المراجعة محفوظًا ويكون تأثيرهما الصافي صفرًا.
                """),
