        ...
        OperationFilterContext context)
    {
        ...
        var documentation = context.MethodInfo.Name switch
        {
            ...
            nameof(InvoicesController.GetItemBalance) => (
                ...
                SwaggerOperationDescription.Create(
                    "Returns the selected item's as-of quantity in `currentQuantity`, weighted-average unit cost in `averageCost`, inventory value, and every item-level advisory pricing expense in `pricingExpenses` (`id`, `name`, per-unit `amount`, and `notes`) for the selected store through the supplied invoice date. `pricingExpensesTotal` is the sum of the separate per-unit expenses, and `totalCostWithPricingExpenses` is the advisory per-unit result of `averageCost + pricingExpensesTotal`. Advisory expenses and the combined cost are display-only and are not included in inventory cost, valuation, invoice totals, or accounting. The legacy `balance` field remains available and always equals `currentQuantity`. Opening balances are represented by active item movements.",
            ...
            nameof(InvoicesController.GetReturnSources) => (
                ...
                SwaggerOperationDescription.Create(
                    ...
                    "Every returned line includes originalQuantity, returnedQuantity, availableQuantity, original unitPrice, originalTotal, costStatus, pendingCostQuantity, and unitCost. Fully returned lines may remain in an eligible invoice for display but have availableQuantity zero. A pending or partially costed source sale can still be returned: the return movement is saved with pending cost and is revalued automatically from the original sale when that cost becomes available. originalDiscountAmount remains informational for the original invoice. Saving the return revalidates availability and enforces the original price. The original invoice-level discount applies only when the current return document includes every original line at its full original quantity; every partial return, including a later remainder after an earlier return, receives zero invoice-level discount.",
