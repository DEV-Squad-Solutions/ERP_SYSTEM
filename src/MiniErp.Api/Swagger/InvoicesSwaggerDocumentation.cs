        ...
        OperationFilterContext context)
    {
        ...
        var documentation = context.MethodInfo.Name switch
        {
            ...
            nameof(InvoicesController.GetById) => (
                "Get an invoice",
                SwaggerOperationDescription.Create(
                    "Returns an invoice aggregate with complete product and container lines plus server-calculated inventory cost status, pending quantity, unit cost, total cost, quantity after, average cost after, and inventory value after for every product movement.",
            ...
            nameof(InvoicesController.Create) => (
                "Create an invoice",
                SwaggerOperationDescription.Create(
                    "Admin only. Creates the complete invoice aggregate, stable item movements, weighted-average cost snapshots, pending-cost allocations, current item/store balances, container movements, partner movement, and driver trip in one Serializable transaction.",
                    ...
                    "Purchase lines use `price` as inbound inventory cost. Sales and purchase returns are outbound at the current weighted average. A sales-return line may send `sourceInvoiceLineId`; the source sale must be fully costed. An unlinked sales return uses the current positive average, or requires `returnUnitCost` when no positive average exists. Cost snapshots, pending quantities, allocations, average cost, and inventory value are server-calculated and must not be sent.",
            ...
            nameof(InvoicesController.Update) => (
                ...
                SwaggerOperationDescription.Create(
                    ...
                    "A stale token returns 409 (`Invoices.Concurrency`) and requires reloading the invoice. Missing or other-company records return 404. Return and relationship rules are the same as create. A sales-return line may retain an optional original Sales-line costing link; this is not posting or invoice allocation.")),
            nameof(InvoicesController.Delete) => (
                "Delete an invoice",
                SwaggerOperationDescription.Create(
                    "Admin only. Soft-deletes the invoice, lines, and synchronized side effects, validates the resulting stock timeline, and rebuilds costing snapshots, allocations, and balances atomically.",
                    ...
                    "Missing, deleted, and other-company invoices return 404. Inbound deletion returns a stock conflict when removing it would make a later historical balance negative. A Sales invoice referenced by an active linked sales return returns 409 (`Invoices.LinkedSalesReturnsExist`). There is no status, posting, cancellation, reversal, voucher, or payment-allocation workflow.")),
            _ => default
        };
