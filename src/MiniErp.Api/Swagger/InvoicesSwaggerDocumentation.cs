        ...
        OperationFilterContext context)
    {
        ...
        var documentation = context.MethodInfo.Name switch
        {
            ...
            nameof(InvoicesController.GetItemBalance) => (
                ...
                SwaggerOperationDescription.Create(
                    "Returns the selected item's as-of quantity in `currentQuantity`, weighted-average cost in `averageCost`, inventory value, and every item-level advisory pricing expense in `pricingExpenses` (`id`, `name`, per-unit `amount`, and `notes`) for the selected store through the supplied invoice date. Advisory expenses are display-only and are not included in inventory cost, valuation, invoice totals, or accounting. The legacy `balance` field remains available and always equals `currentQuantity`. Opening balances are represented by active item movements.",
