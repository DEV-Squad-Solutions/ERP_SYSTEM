        ...
        OperationFilterContext context)
    {
        ...
        var documentation = context.MethodInfo.Name switch
        {
            ...
            nameof(InvoicesController.Create) => (
                "Create an invoice",
                SwaggerOperationDescription.Create(
                    "Admin only. Creates the complete invoice aggregate, stable item movements, weighted-average cost snapshots, pending-cost allocations, current item/store balances, container movements, full invoice partner movement, optional generated payment voucher and opposite partner payment movement, and driver trip in one Serializable transaction.",
                    "A required `invoiceNumber` of at most 100 characters, optional `partnerInvoiceNo` of at most 100 characters, plus `invoiceType`, `contentType` (`Items` or `Containers`), `invoiceDate`, `businessPartnerId`, `storeId`, the matching `lines` or `containerLines` collection, `discountAmount`, `paidAmount`, and a required `paymentTerm` select (`Cash` or `Credit`). Cash requires `paidAmount == total`; Credit accepts zero or a partial amount only. Every positive paid amount requires `cashboxId` and `cashMovementTypeId`; both must be null when paid amount is zero.",
            ...
            nameof(InvoicesController.Update) => (
                ...
                SwaggerOperationDescription.Create(
                    ...
                    "Invoice number is immutable after creation. PartnerInvoiceNo is editable. Currency, item units, quantities, line totals, subtotal, invoice total, payment status, remaining amount, audit fields, and company ID are server-controlled. Cash remains fully paid; Credit remains zero or partially paid. A positive paid amount requires the current cashbox and movement-type selections. The generated voucher and its partner movement are updated in place while payment remains positive, created when payment becomes positive, and soft-deleted when payment becomes zero.",
