        ...
        var documentation = context.MethodInfo.Name switch
        {
            ...
            nameof(InvoicesController.Create) => (
                ...
                SwaggerOperationDescription.Create(
                    ...
                    "The product store, partner, items, item units, optional country, and driver must be active and belong to the selected company. Container lines are sales-only and require the selected partner's active container store and assigned active containers. Returns are independent invoice documents and do not reference an earlier invoice. Do not send invoice number, currency, item unit, subtotal, total, remaining amount, calculated line amounts, row version, or company ID. For Cash, `paidAmount` must equal the net total; for Credit, it may be zero through the net total.",
            ...
            nameof(InvoicesController.Update) => (
                ...
                SwaggerOperationDescription.Create(
                    "Admin only. Replaces editable header, product-line, and container-line fields in one serializable transaction. Discount and paid amounts are replaced with the header; partner movement side effects are recreated from the new remaining amount. Child-only changes touch the header and advance its row version.",
