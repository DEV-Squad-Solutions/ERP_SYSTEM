        ...
        var documentation = context.MethodInfo.Name switch
        {
            ...
            nameof(InvoicesController.Update) => (
                ...
                SwaggerOperationDescription.Create(
                    ...
                    "Invoice number is immutable after creation. PartnerInvoiceNo and the three weighbridge inputs are editable. `wbTotal`, currency, item units, quantities, line totals, subtotal, invoice total, payment status, remaining amount, base-currency snapshots, audit fields, and company ID are server-controlled. Cash remains fully paid; Credit remains zero or partially paid. A positive paid amount requires the current cashbox and movement-type selections. The generated voucher and its partner movement are updated in place while payment remains positive, created when payment becomes positive, and soft-deleted when payment becomes zero.",
