        ...
        var documentation = context.MethodInfo.Name switch
        {
            ...
            nameof(InvoicesController.Update) => (
                ...
                SwaggerOperationDescription.Create(
                    ...
                    "Invoice number is immutable after creation. PartnerInvoiceNo, itemsCategoryId, and the three weighbridge inputs are editable. For item lines, send either `count` and `weight`, or omit both and send a positive `quantity` directly. Every item-invoice replacement must keep the calculated `wbTotal` equal to the sum of effective line weights, including when all weighbridge inputs are zero; container invoices are excluded. The category must be active and company-owned when newly selected; an already-linked category may remain on an invoice after deactivation. `itemsCategoryName`, `wbTotal`, currency, item units, quantities, line totals, subtotal, invoice total, payment status, remaining amount, base-currency snapshots, cash movement type, audit fields, and company ID are server-controlled. Cash remains fully paid; Credit remains zero or partially paid. A positive paid amount requires the current cashbox. The generated voucher and its partner movement are updated in place while payment remains positive, created when payment becomes positive, and soft-deleted when payment becomes zero. An existing compatible voucher keeps its historical movement type; otherwise the server uses the current default.",
