    ...
    private static (string, string) GetAccountsDocumentation(string methodName) =>
        methodName switch
        {
            ...
            nameof(AccountsController.GetJournalSelect) => (
                ...
                SwaggerOperationDescription.Create(
                    "يعيد الحسابات الفرعية الفعالة المتاحة للقيد اليدوي أو التسوية أو القيد الافتتاحي في سنة مالية محددة، بما في ذلك الحسابات المرتبطة بعناصر تشغيلية، لتجنب تسجيل نفس الأثر مرتين.",
                    "أرسل `fiscalYearId` في query.",
                    "السنة يجب أن تكون موجودة في الشركة الحالية. كل حساب يعيد `partyGroups`: العملاء والموردون يستخدمون نفس قائمة الأطراف الفعالة كلٌ تحت تصنيفه، والموظفون والسائقون يظهرون تحت حساباتهم المستقلة، والخزائن تظهر فقط إذا كان لها AccountMapping من النوع `Cashbox` و`sourceId` يساوي الخزينة.",
                    "الحساب المرتبط بذمم العملاء أو الموردين أو الموظفين أو السائقين أو الخزائن لا يمكن التسجيل عليه دون اختيار طرف مطابق. التجميع المالي يعتمد `accountId` وبيانات الطرف بُعد تحليلي. تجنب تسجيل نفس أثر الحركة التلقائية يدويًا.")),
